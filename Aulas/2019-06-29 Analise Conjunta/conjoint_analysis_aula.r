...
str(tprefm) #100 respondentes para 13 estímulos
...
#Share: percentual do valor do estímulo
...
sportscar <- mlogit.data(sportscar_long,
                       ...
                       choice = "choice")
head(sportscar)
#É preciso transformar os dados em "mlogit.data"
#Shape, alt.var e choice são decididos nessa formatação dos dados.
...
# modify the code below so that the segment is set to "racer" for both alternatives
...
#Share é a preferência. É função das estimativas dos níveis dos fatores (como na regressão).
...
#Wide: cada observação é uma questão com várias opções.
#Long: cada observação é uma opção; a mesma questão aparece em mais observações.
#Long é preferível neste caso.
...
chocolate_long <- read.csv("chocolate_choice_long.csv") # esse Ã© o dado bruto
# ver se sao iguais
head(chocolate_long)
...
# O intercepto nÃ£o Ã© identificado.
...
##ïƒ  Divida os coeficientes pelo
...
# (razao do coeficiente `BrandGhirardelli` sobre o negativo do coeficiente de preÃ§o - coef_price).
# Ou seja, se Ghirardeli pode custar ate 1.56 a mais que Dove q as pessoas preferem essa marca
# Veja `TypeWhite` e interprete
# É dado o preço que poderia ser cobrado a mais ou a menos, em comparação às marcas ou tipos zero, para igualar as utilidades.


##########
## Ver a interacao `Trial` e `Price`: quantas tentativas já foram feitas.

choc_m3 <- mlogit(Selection ~ 0 + Brand + Type + Price + Price:Trial, data = chocolate)
summary(choc_m3)
# Ao longo das escolhas, o respondente poderia estar escolhendo os mais baratos, caso fosse significante.
