...
Para as aulas usaremos os pacotes `tidyverse`, para manipulação e observação dos dados. `NbClust` e `cluster` (prox aula) para análise de clusters.
...
Uma tabela com 7 observações de 5 variáveis (dados obtidos de Hair et al.)
...
Veja que as observações 1, 5 e 6 são altamente correlacionadas. Visualizar isso no gráfico:
...
As correlações negativas fortes são observadas entre as observações 3 e 4:
...
Logo, correlações não veem as similaridades pelos valores, mas pelo comportamento das variáveis.

Uma forma de calcular a similaridade é por seu inverso: *dissimilaridade*, que pode ser:
...
A distância Euclidiana usa a hipotenusa de um triangulo retangulo (para 2 pontos separados). usando o exemplo dos jogadores vermelho e azul:
...
```
    ...
    - Distância Máxima (Chebychev):

```{r}
dist(jogadores, 'maximum')
```

Comparando matriz de correlação e matriz de distâncias:
...
A distância de mahalanobis leva em consideração a variância-covariância das (correlações entre as) variáveis. Portanto, leva em consideração a distância do centroide mas também a variância (elipse dos pontos).
...
Usando a função `cutree`, voce estabelece a altura (`h=`, *height*) ou número de grupos que você considera ideal (*k groups*, `k=`):
...
Como existe uma certa correlação entre as variáveis de pétalas e sépalas dentro das espécies, o uso de correlação pode fazer sentido.

Para uma melhor discussão sobre o melhor modo de calcular dissimilaridade: http://research.stowers.org/mcm/efg/R/Visualization/cor-cluster/index.htm
...
Confrontando com as espécies reais:
...
# Faça kmeans com dois numeros de "sementes" diferentes: 1 e 2:
...
Ver qual o valor de Silhouette médio dos grupos com o gráfico (Pelo pacote `factoextra`). O valor médio deve ser o maior possível:
...
2. Método de determinar eps ótimo
...
Os comandos a seguir criam um gráfico que compara os três métodos (Ward, UPGMA e UPGMC) relacionando a variância das distâncias (soma dos quadrados de dissimilaridade) em função do número de grupos.
...
#Verifique a razão entre SQ intragrupos e total e intergrupos
dados_km_1$tot.withinss/dados_km_1$betweenss
dados_km_2$tot.withinss/dados_km_2$betweenss
dados_km_4$tot.withinss/dados_km_4$betweenss
...
Ver teoria de grafos: Nós, Vértices e ligações
...
A partir de matriz de adjacência
...
plot(g)
```
Use `V()` e `E()` para ver os vértices e ligações da rede `g`.
Use `gsize()` para contar o número de ligações em uma rede.
Use `gorder()` para contar o número de vértices em uma rede.
...
Diâmetro - Observações mais periféricas
```{r}
get_diameter(g)
```
...
1. Gerar 1000 gráficos aleatórios baseados na rede original. Ou seja: com o mesmo número de vértices e densidade aproximada
2. Calcular a média do comprimento dos caminhos da rede original.
3. Calcular a média do comprimento dos caminhos das 1000 redes aleatórias
4. Determine quantas redes aleatórias mostrar valores maiores ou menores que a rede original
