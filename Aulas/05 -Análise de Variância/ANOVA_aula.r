...
dados <- data.frame(valor = c(20+rnorm(30), 22+rnorm(30)),
                    plantacao = c(rep("p1",30), rep("p2",30)))
 write.csv(dados, "dados.csv")
...
#Exerc�cios IRIS
data(iris)
#   Hip�tese nula: m�dias dos comprimento das p�talas
#  Petal.Lenght ~ Species, data=iris

#1. Distribui��o normal
shapiro.test(iris$Petal.Length) #n�o � normal
...
tapply(iris$Petal.Length, iris$Species, shapiro.test) #une os tr�s anteriores
tapply(
  X = iris$Petal.Length,
  INDEX = iris$Species,
  FUN = shapiro.test) #igual ao anterior, por argumento
...
#6. Teste post hoc (Tukey); apenas se a ANOVA foi significativa
TukeyHSD(anova_iris)
plot(TukeyHSD(anova_iris))

#7. Avalia��o de res�duos (demonstra��o da homocedastidade)
plot(anova_iris,which=1) #erro within
plot(anova_iris,which=2) #erro betwin

########################
#EXERC�CIO ANOVA 2 WAYS#
########  CO2   ########
########################
...
#ANCOVA consisiderando concentra��o
data("CO2")
CO2
