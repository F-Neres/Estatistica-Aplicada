...
#setwd("C:/Users/User/Documents/Disciplinas/Pós estatistica/11 - MANOVA e ANCOVA/aulas")
hatco<-read.csv('HATCO.csv',sep=",", dec=".", header = T)
str(HATCO)
hatco <- HATCO
...
biotools::boxM(cbind(hatco$X9,hatco$X10), hatco$X8) # equivalencia das matrizes var-covar; se significativo, a matriz não é igual; heterocedástico

if(!require(DescTools)){install.packages("DescTools")}
with(hatco, 
     DescTools::HotellingsT2Test(
       cbind(X9,X10) ~ X8)) #Teste T² de Hotelling. Como o F para a ANOVA. H0 assume que as médias são todas iguais; não há qualquer sinal.
#X9 e X10 são métricas. X8 é a resposta categórica.

summary(manova(lm(cbind(X9,X10) ~ X8, data=hatco))) #T² de Hotelling, mas dado como F aproximado. Não exige o pacote DescTools
...
#Nível de uso (X9) em relação a tamanho (X8: grande ou pequeno) e tipo (X13: indústria ou outros)
...
# Teste Post HOC
modelo <- aov(lm(X9 ~ X8*X13, data=hatco)) #Errado, por a interação não é significativa.
modelo <- aov(lm(X9 ~ X8, data=hatco)) #Já que apenas X8 foi significativo
TukeyHSD(modelo) # se desse ANOVA significativa. Como só há dois níveis, o teste é redundante, por obviamente será significativo.
#Seria mais interessante com pelo menos 3 níveis, por o teste te Tukey faz testes par-a-par.
...
## add interaction means - Adiciona as médias das interações de X9 e X10.
...
print(car::linearHypothesis(modelo, 
                            c("X8small", "X13other"), 
                            title="Main effects"), SSP = FALSE) #Sem interação

print(car::linearHypothesis(modelo, c("X8small", "X13other", "X8small:X13other"),
                            title="Groups"), SSP = FALSE) #Com interação
...
# Intercept = 40.050 , X8small = 8.883
summary(aov(passo1))  # Pegar F e p-valor
# F = 15.142 , p-value = 0.000184
...
# Teste de inferencia para as interacoes (declives não-homogeneos)
#anova(aov(X9 ~ X7 * X8, data = hatco))

### Executar e interpretar ANCOVA
mod <- lm(X9 ~ X7*X8, data = hatco) # Ordem dos fatores é importante !
summary(mod)
anova(mod)
