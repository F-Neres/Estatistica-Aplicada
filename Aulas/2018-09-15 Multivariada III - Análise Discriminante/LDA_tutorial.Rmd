...
A **Regressão Logística** também tem o objetivo de usar variáveis independentes e produzir como resposta o grupo a qual um objeto pertence (1 ou 0, p.ex), mas a LDA:
  ...
  - Usa regressão linear mas é interpretada de maneira diferente (Grupos ~ X1 + X2...Xn), pois os "eixos são simplificados" (Funções Discriminantes que são construídas a partir das variáveis)
...
Portanto, a LDA tem como principais **objetivos**:
 ...
 - Escolher variáveis independentes que ajudem a explicar o máximo de diferenças no escores.iris dos grupos
 - Estabelecer procedimentos para classificar objetos basendo-se em um conjunto de variáveis independentes

## Pressupostos da LDA

Os principais pressupostos para essa análise é:
  ...
  - As **matrizes de variância-covariância iguais** entre os grupos;
  - O tamanho da **amostra** deve ser considerável (1. mínimo 5 observações por variável independente; 2. cada grupo deve ter mais objetos que variáveis independentes analisadas, 3. grupos com tamanhos semelhantes).
  
## LDA passo-a-passo

Para a execução de uma LDA, sabendo claramente de qual é seu objetivo, o pesquisador deve seguir **6 passos**.

  1. Tamanho da Amostra e Criação de amostras "treinoiris" e "testeiris"

  2. Seleção e Análise das variáveis independentes
...
Como mantém um número razoável por grupo (50) e só temos 4 variáveis independentes, podemos fazer uma divisão 50:50 para amostras "treinoiris" e "testeiris", ou até um pouco maior para "treinoiris" e o restante para "testeiris" (2:1, por exemplo).
...
As proporções ficaram um pouco diferentes, se quiser testar se as proporções são semelhantes, você pode aplicar um testeiris de proporções com $\chi^2$:
...
Caso você tenha muitas variáveis (mais do que tem de observações em cada grupo), é interessante reduzir a quantidade de variáveis a serem utilizadas.
O procedimeto a ser utilizado pode ser por avaliação das variáveis por seus pressupostos e correlações, mas também pode ser utilizado o procedimento de *stepwise*. 
...
[3](https://www.rdocumentation.org/packages/klaR/versions/0.6-14/topics/stepclass) Pacote `klaR` com a função `stepclass()`
...
A seguir, exploraremos os pressupostos das variáveis independentes: (A) Normalidade , (B) Linearidade das relações, (C) Multicolinearidade, (D) Igualdade das Matrizes de dispersão entre grupos:
...
Faça uma verificação visual das relações entre as variáveis, para ver se alguma das relações mostra-se com alguma relação nao linear (p.ex: quadrática, exponencial, etc).
Pessoalmente, gosto de fazer essa análise visual com o pacote `GGally` , muito similar ao `ggplot2`:
...
cor(treinoiris[,1:4])
```
A qual podemos verificar pela correlação alta = `r cor(treinoiris$Petal.Width, treinoiris$Petal.Length)`.

Se tivéssemos muitas variáveis, essas duas variáveis poderiam ser sintetizadas em uma única, portanto você poderia utilizar somente `Petal.Width` ou `Petal.Length`, por exemplo.
Nós manteremos o uso das duas variáveis, pois o nosso modelo LDA pode perder poder de discriminação se usarmos poucas variávels.

#### D. Matrizes de dispersão iguais

As Matrizes de variâncias-covariâncias devem ser iguais entre os grupos. Para isso, podemos executar o testeiris M de Box. Para executarmos uma LDA, amostras pequenas e matrizes de covariância desiguais afetam negativamente a significância do processo de estimação das funções discriminantes.

O pacote `biotools` possui a função `boxM()` para que possamos executar esse testeiris:
...
Infelizmente o testeiris M de Box (Box's M test) é muito sensível à violações da normalidade, levando a rejeição de grande parte dos casos, que é o nosso caso (p-value <0.01, logo rejeitamos a hipótese nula de que as matrizes são iguais entre os grupos).

Quando as matrizes são diferentes, a sugestão é (i) utilizar QDA ao invés de LDA ou (ii) fazermos uma validação cruzada dos dados, com a separação das amostras emm `treinoiris` e em `testeiris`, como fizemos, e testar se o modelo construído tem um bom ajuste (verificar tabela confusão)
...
com o argumento `CV=TRUE`, a função `lda()` executa um *cros-validation*. A principal diferença de *output* é que com `CV=TRUE` há uma tabela no resulta.irisdo da LDA chamado `posterior` (`dis2$posterior`), que são as **probabilidades** de serem classificadas em cada uma das espécies (*Iris setosa*, *I. versicolor* e *I. virginica*) pela FD (função de Classificação, também chamada de Função Discriminante Linear de Fisher) construída.

Esse objeto `posterior` é muito semelhante (se a sua FD estiver boa) com o resulta.irisdo da aplicação da FD nos dados inseridos (`predict(dis1)`), que também possui uma tabela `posterior`, com as probabilidades de cada observação pertencer a um determinado grupo (`setosa`, `versicolor` ou `virginica`):
...
Essas probabilidades são obtidas pelos valores das funções discriminantes (escores.iris) obtidas para cada objeto (observação). Esses valores podem ser obtidos na tabela `x` do objeto `previsto` (resulta.irisnte do `predict(dis1)`)
...
Os escores.iris dessas funções discriminantes (`LD1` e `LD2`) são importantes para fazer as separações dos grupos. O escore discriminante é `0` para cada um dos LD, portanto, cada grupo é separado pelo seu sinal. A primeira LD (`LD1`) separa 1 grupo do restante dos grupos. Um objeto será classificado de 1 grupo determinado se apresentar valor positivo ou pertencerá ao restante se apresentar valor negativo. As LDs vão sendo construídas até conseguirmos separar todos os grupos do restante. Por isso que o número de LDs será $N-1$. 
...
Para isso será necessário ver os escores.iris das **cargas discriminantes** (correlações das variáveis independentes com as LD -funções discriminantes - construídas). os valores estão contidos no objeto resulta.irisnte da `lda()` na tabela `scaling`:
...
Podemos observar com o gráfico dos escores.iris.iris e seus centróides (+) para cada espécie:
...
Para verificar se as LDs apresentam significância, podemos utilizar o valor de Lambda de Wilk:
Testa a significancia estatistica do poder discriminatório da(s) função(ões) discriminante(s). Wilks' lambda: varia de 1.0 (sem poder discriminatorio) ate 0.0 (poder discriminatorio perfeito).
A hipótese nula neste caso é que *não há diferença significativa entre os centroides dos grupos*
...
Podemos verificar se a nossa amostra `testeiris` também responderá com um bom desempenho para a FD construída com os dados de `treinoiris` (verificando o *overfitting*):
